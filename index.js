...
const prepareClause = (config) => {
  ...
  if (where) {
    ...
    if (where.clause) {
      let clause = "%CLAUSE_DATA%";
      where.clause.forEach((c) => {
        if (where.node || where.class) {
          if (where.node) {
            clause = clause.replace(
              "%CLAUSE_DATA%",
              `{${where.node}:%CLAUSE_DATA%}`
            );
          }
          ...
          if (where.class) {
            clause = clause.replace(
              "%CLAUSE_DATA%",
              `{${where.class}:{${c.field}:{_${c.operator}: ${getParamValue(
                c
              )}}}}`
            );
          }
      ...
      res = res.replace("%DATA%", `${clause}`).replace("%CLAUSE_DATA%", "");
    }
  }
